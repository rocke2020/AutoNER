...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_dataset', default='./data/hqner/test.pk')
    parser.add_argument('--train_dataset', default='./data/hqner/train_0.pk')
        ...
        for indexs in range(args.epoch):
            ...
            for word_t, char_t, char_mask, chunk_gap_ids, type_mask, type_ids in train_loader.get_tqdm(device):
                ...
                if 0 == batch_index % args.check:
                    ...
                    pw.save_checkpoint(model = ner_model, is_best = f1_dev > best_eval, s_dict = {
                        'config': ner_config, 'w_map': w_map, 'c_map': c_map, 'tl_map': tl_map, 'cl_map': cl_map})
                    ...
                    if f1_dev > best_eval:
                        ...
                        for entity_type in best_type2f1:
                            pw.add_loss_vs_batch({'per_{}_f1'.format(entity_type): best_type2f1[entity_type], 
                                                    ...
                                                    'per_{}_rec'.format(entity_type): best_type2rec[entity_type]},
                                                    batch_index, use_logger = False)
                            logger.info('\ttype: %s, f1: %.6f, pre: %.6f, rec: %.6f' % (
                                entity_type, best_type2f1[entity_type], 
                                best_type2pre[entity_type], 
        ...
        if f1_dev > best_eval:
            ...
            for entity_type in best_type2f1:
                pw.add_loss_vs_batch({'per_%s_f1'.format(entity_type): best_type2f1[entity_type], 
                                        ...
                                        'per_%s_rec'.format(entity_type): best_type2rec[entity_type]}, 
                                        batch_index, use_logger = False)
                logger.info('\ttype: %s, f1: %.6f, pre: %.6f, rec: %.6f' % (
                    entity_type, 
                    best_type2f1[entity_type], 
                    best_type2pre[entity_type], 
                    best_type2rec[entity_type]))
